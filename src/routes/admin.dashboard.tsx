...
function AdminDashboard() {
  ...
  const [recent, setRecent] = useState<
    { id: string; title: string; status: string; updated_at: string; slug: string }[]
  >([]);

  useEffect(() => {
    (async () => {
      const counts = async (
        table: "articles" | "comments" | "saved_articles" | "profiles" | "reading_history",
      ...
      const [total, published, drafts, scheduled, comments, saves, users, reads] =
        await Promise.all([
          counts("articles"),
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Overview of publishing activity and audience signals.
          </p>
        </div>
        <Link
          to="/admin/new"
          className="inline-flex items-center gap-1.5 h-10 px-4 rounded-sm bg-primary text-primary-foreground font-semibold"
        >
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <section className="lg:col-span-2 rounded-sm border border-border">
          ...
          <ul className="divide-y divide-border">
            {recent.map((r) => (
              <li key={r.id} className="p-4 flex items-center gap-3">
                ...
                <Link
                  to="/admin/edit/$id"
                  params={{ id: r.id }}
                  className="font-serif font-semibold flex-1 hover:underline"
                >
                  {r.title}
                </Link>
            ...
            {recent.length === 0 && (
              <li className="p-6 text-sm text-muted-foreground">No articles yet.</li>
            )}
          </ul>
        </section>
        ...
        <section className="rounded-sm border border-border">
          ...
          <ul className="p-4 space-y-3">
            ...
            {trending.length === 0 && (
              <li className="text-sm text-muted-foreground">No published content.</li>
            )}
          </ul>
        </section>
      </div>
