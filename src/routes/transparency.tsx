...
async function loadTransparencyData() {
  try {
    ...
    const { count: correctionCount, error: correctionError } =
      await supabase
        .from("corrections")
        .select("*", {
          count: "exact",
    ...
    const { data: corrections, error: recentError } = await supabase
      .from("corrections")
      .select(
        "id,note,created_at,editor_name,article_id"
      )
    ...
    setStats({
      ...
      verified: articles.filter(
        (article) => article.verification === "Verified"
      ).length,
      factChecked: articles.filter(
        (article) => article.verification === "Fact-Checked"
      ).length,
      developing: articles.filter(
        (article) => article.verification === "Developing"
      ).length,
      opinion: articles.filter(
        (article) => article.verification === "Opinion"
      ).length,
...
Public Accountability </div>
      ...
      <p className="mt-4 text-lg text-primary-foreground/80 max-w-2xl leading-8">
        A public view of ClearFact's verification record, corrections,
        editorial classifications and trust metrics.
      </p>
      ...
      <div className="mt-6 space-y-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 leading-6">
            Published information that has undergone ClearFact's editorial
            verification process and is considered sufficiently supported
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 leading-6">
            A report or claim that has undergone additional fact-checking
            against relevant sources, records or evidence.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 leading-6">
            A developing story where information may continue to change as
            authorities, witnesses or other credible sources provide new
            information.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 leading-6">
            Commentary, analysis or other material that is clearly
  ...
  <section className="container-news pb-12">
    ...
    <p className="mt-3 text-sm text-muted-foreground max-w-2xl">
      We believe corrections should be transparent. When a material error
      is identified, ClearFact may correct the relevant article and record
    ...
    {recent.length === 0 ? (
      <div className="mt-6 border border-border rounded-sm p-6">
        <p className="text-sm text-muted-foreground">
          No public corrections have been logged in the current
          transparency record.
        </p>
      </div>
    ) : (
  ...
  <section className="container-news pb-14">
    <div className="rounded-sm border border-border bg-card p-6 md:p-8">
      ...
      <h2 className="font-serif text-3xl mt-2">
        How our transparency data is generated
      </h2>

      <p className="text-sm text-muted-foreground mt-4 leading-6">
        The dashboard displays information recorded in ClearFact's
        newsroom systems for published articles and logged corrections.
        Verification labels and trust scores are editorial classifications
        and should be understood as part of ClearFact's internal review
        process rather than as an absolute guarantee of truth.
      </p>

      <p className="text-sm text-muted-foreground mt-3 leading-6">
        Metrics may change as articles are updated, reclassified or
        corrected. The dashboard is intended to provide readers with
        greater visibility into our editorial processes.
      </p>
    </div>
  </section>
  ...
  <section className="container-news pb-16">
    <div className="border-t border-border pt-8">
      ...
      <p className="text-sm text-muted-foreground mt-2 leading-6">
        If you believe an article contains an error or you have a question
        about our verification process, contact the ClearFact editorial
        desk.
      </p>
