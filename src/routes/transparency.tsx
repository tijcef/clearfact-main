  ...
  head: () => ({
    meta: [
      { title: "Transparency Dashboard — ClearFact News" },
      {
        name: "description",
        content:
          "Live, public verification metrics, corrections, source disclosure and editorial accountability for ClearFact News.",
      },
  ...
  useEffect(() => {
    (async () => {
      ...
      setStats({
        ...
        avgTrust: a.length
          ? Math.round(a.reduce((s, x) => s + (x.trust_score ?? 70), 0) / a.length)
          : 0,
      });
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="container-news py-14 md:py-20">
          ...
          <p className="mt-4 text-lg text-primary-foreground/80 max-w-2xl">
            Live verification record, correction history and trust metrics. Truth over speed.
            Verification over virality. Trust over traffic.
          </p>
          <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {cards.map((c) => (
              ...
              >
                ...
                <div className="text-xs text-primary-foreground/70">{c.l}</div>
              </div>
            ))}
