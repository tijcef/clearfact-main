...
function AdminPayouts() {
  ...
  const decide = async (w: W, decision: "approve" | "reject" | "mark_paid" | "mark_failed") => {
    ...
    try {
      await decideFn({
        data: { withdrawalId: w.id, decision, note: notes[w.id], gatewayReference: refs[w.id] },
      });
  ...
  return (
    <div className="container-news py-8 space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Approve, reject and mark contributor withdrawals as paid.
          </p>
        </div>
        <select
          value={filter}
      ...
      {items.length === 0 && (
        <p className="text-muted-foreground py-6">No requests in this view.</p>
      )}
        ...
        {items.map((w) => {
          ...
          return (
            <div key={w.id} className="rounded-sm border border-border bg-card p-5">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {a
                      ? `${a.provider} · ${a.display_name}${a.account_number_masked ? ` · ${a.account_number_masked}` : ""}`
                      : "Unknown account"}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Fee {fmt(w.fee_kobo)} · Tax {fmt(w.tax_kobo)} · Net{" "}
                    {fmt(w.amount_kobo - w.fee_kobo - w.tax_kobo)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="text-right">
                  <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-sm font-semibold bg-accent">
                    {w.status}
                  </span>
                  {w.fraud_score > 0 && (
                    <div
                      className={`mt-2 inline-flex items-center gap-1 text-xs ${w.fraud_score >= 40 ? "text-breaking" : "text-muted-foreground"}`}
                    >
              ...
              {flags.length > 0 && (
                <div className="mt-3 flex gap-1.5 flex-wrap">
                  {flags.map((f) => (
                    <span
                      key={f}
                      className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-sm bg-breaking/10 text-breaking"
                    >
                      {f.replace(/_/g, " ")}
                    </span>
                  ))}
              ...
              <div className="mt-3 flex gap-2 flex-wrap">
                {w.status === "pending" && (
                  <>
                    <button
                      ...
                      className="h-9 px-3 rounded-sm bg-verified text-background text-sm font-semibold inline-flex items-center gap-1.5 disabled:opacity-60"
                    >
                      {busy === w.id ? (
                    ...
                    <button
                      disabled={busy === w.id}
                      onClick={() => decide(w, "reject")}
                      className="h-9 px-3 rounded-sm bg-breaking text-breaking-foreground text-sm font-semibold inline-flex items-center gap-1.5 disabled:opacity-60"
                    >
                ...
                {w.status === "approved" && (
                  <>
                    <button
                      disabled={busy === w.id}
                      onClick={() => decide(w, "mark_paid")}
                      className="h-9 px-3 rounded-sm bg-primary text-primary-foreground text-sm font-semibold inline-flex items-center gap-1.5 disabled:opacity-60"
                    >
                      <Send className="h-3.5 w-3.5" /> Mark paid
                    </button>
                    <button
                      disabled={busy === w.id}
                      onClick={() => decide(w, "mark_failed")}
                      className="h-9 px-3 rounded-sm bg-breaking/15 text-breaking text-sm font-semibold inline-flex items-center gap-1.5 disabled:opacity-60"
                    >
              ...
              {w.reviewer_note && (
                <div className="mt-3 text-xs text-muted-foreground">Note: {w.reviewer_note}</div>
              )}
              {w.gateway_reference && (
                <div className="mt-1 text-xs text-muted-foreground font-mono">
                  Ref: {w.gateway_reference}
                </div>
              )}
            </div>
          );
