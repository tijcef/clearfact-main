...
function WalletView() {
  ...
  const totalEarned = entries
    .filter((e) => e.kind === "earning" || e.kind === "bonus")
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="rounded-sm border border-border bg-card p-5">
          ...
          <Link
            ...
            className="mt-3 inline-flex h-9 px-4 rounded-sm bg-primary text-primary-foreground text-sm font-semibold items-center"
          >
            Request payout
          </Link>
        </div>
        ...
        <div className="divide-y divide-border">
          ...
          {entries.map((e) => (
            <div key={e.id} className="p-4 flex items-center gap-3">
              <div className="h-9 w-9 grid place-items-center rounded-sm bg-accent/40">
                {ICON[e.kind]}
              </div>
              ...
              <div
                className={`font-mono font-semibold ${e.kind === "payout" ? "text-breaking" : "text-verified"}`}
              >
                {e.kind === "payout" ? "−" : "+"}
                {fmt(e.amount_kobo)}
              </div>
