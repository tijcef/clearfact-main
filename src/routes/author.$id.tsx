...
function AuthorPage() {
  ...
  if (profile === undefined)
    return (
      <div className="container-news py-16 flex items-center gap-2 text-muted-foreground">
    ...
    <div className="container-news py-10">
      <header className="flex items-start gap-5 pb-6 border-b border-border">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs uppercase tracking-wider text-primary font-semibold mt-2">
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        {articles.map((a) => (
          <article key={a.id} className="border-b border-border pb-5">
            {a.cover_image && (
              <Link to="/post/$slug" params={{ slug: a.slug }}>
                <img
                  ...
                  alt=""
                  loading="lazy"
                  className="aspect-[16/10] w-full object-cover rounded-sm mb-3"
                />
              </Link>
            )}
            ...
            <Link to="/post/$slug" params={{ slug: a.slug }}>
              <h2 className="font-serif text-xl mt-1 hover:underline decoration-gold underline-offset-4">
                {a.title}
              </h2>
            </Link>
            <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{a.excerpt}</p>
            <div className="mt-2 text-xs text-muted-foreground">
              {a.published_at && new Date(a.published_at).toLocaleDateString()} · {a.read_minutes}{" "}
              min read
            </div>
          </article>
        ))}
