...
const SPECIALTIES: { value: Specialty; label: string; desc: string }[] = [
  {
    ...
    desc: "Eyewitness accounts and community news.",
  },
  {
    value: "freelance_journalist",
    label: "Freelance Journalist",
    desc: "Independent reporting across beats.",
  },
  {
    value: "verified_correspondent",
    label: "Verified Correspondent",
    desc: "Embedded reporter for a region.",
  },
  {
    value: "investigative_reporter",
    label: "Investigative Reporter",
    desc: "Long-form, evidence-heavy work.",
  },
  ...
  return (
    <div className="container-news py-10 max-w-2xl">
      ...
      <p className="text-muted-foreground mt-2">
        All ClearFact contributors complete identity verification. Your documents are encrypted and
        only visible to ClearFact editors.
      </p>
      ...
      <ol className="flex items-center gap-2 my-8 text-xs">
        {steps.map((s, i) => (
          <li key={s} className="flex items-center gap-2">
            <span
              className={`h-7 w-7 grid place-items-center rounded-full font-semibold ${i + 1 <= step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              {i + 1}
            </span>
      ...
      <div className="rounded-sm border border-border bg-card p-6 space-y-5">
          ...
          <>
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {SPECIALTIES.map((s) => (
                  <button
                    ...
                    className={`text-left p-3 rounded-sm border ${specialty === s.value ? "border-primary bg-accent" : "border-border hover:bg-accent/50"}`}
                  >
            ...
            <Field label="Bio">
              <textarea
                ...
                placeholder="Background, beats you cover…"
              />
            </Field>
            <Field label="Location">
              <input
                ...
                placeholder="Yola, Adamawa"
              />
            </Field>
            <div className="flex items-center gap-2 text-xs text-verified">
              <Mail className="h-4 w-4" /> Email verified via signup
            </div>
          </>
        )}
        {step === 2 && (
          <>
            ...
            {!otpSent && (
              <button
                onClick={sendOtp}
                className="h-10 px-4 rounded-sm bg-primary text-primary-foreground font-semibold"
              >
                Send OTP
              </button>
            )}
            {otpSent && !phoneVerified && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  maxLength={6}
                  className="h-10 w-32 px-3 rounded-sm border border-border bg-background tracking-widest text-center"
                  placeholder="••••••"
                />
                <button
                  onClick={verifyOtp}
        ...
        {step === 3 && (
          <>
            <p className="text-sm text-muted-foreground">
              Upload a clear photo of your government-issued ID (NIN slip, voter card, driver's
              licence, passport).
            </p>
            ...
            {idFile && (
              <div className="text-xs text-verified flex items-center gap-1">
                <IdCard className="h-3 w-3" /> {idFile.name} ready
              </div>
            )}
          </>
        )}
        {step === 4 && (
          <>
            <p className="text-sm text-muted-foreground">
              Upload a selfie holding your ID. We compare it to confirm you're the document owner.
            </p>
            <Field label="Selfie">
              <input
                type="file"
            ...
            {selfieFile && (
              <div className="text-xs text-verified flex items-center gap-1">
                <Camera className="h-3 w-3" /> {selfieFile.name} ready
              </div>
            )}
          </>
        )}
        {step === 5 && (
          <>
            <p className="text-sm">
              Review your details and submit. An editor will activate your account within 48 hours.
            </p>
            <ul className="text-sm space-y-1">
              ...
              <li>
                <IdCard className="inline h-3 w-3 mr-1" /> {idFile?.name ?? "—"}
              </li>
              <li>
                <Camera className="inline h-3 w-3 mr-1" /> {selfieFile?.name ?? "—"}
              </li>
            </ul>
          </>
        )}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          <button
            ...
            disabled={step === 1}
            className="h-10 px-4 rounded-sm border border-border disabled:opacity-50"
          >
            Back
          </button>
          {step < 5 ? (
            <button
              ...
              className="h-10 px-5 rounded-sm bg-primary text-primary-foreground font-semibold"
            >
              Next
            </button>
          ) : (
            <button
              onClick={finish}
              disabled={busy}
              className="h-10 px-5 rounded-sm bg-primary text-primary-foreground font-semibold inline-flex items-center gap-2 disabled:opacity-60"
            >
