...
function PayoutsView() {
  ...
  const addAccount = async () => {
    ...
    setBusy(true);
    const masked = form.account_number
      ? form.account_number.slice(-4).padStart(form.account_number.length, "•")
      : null;
  ...
  const submitWithdrawal = async () => {
    ...
    try {
      await requestFn({
        data: { payoutAccountId: withdrawForm.accountId, amountKobo: Math.round(naira * 100) },
      });
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage payout destinations and request withdrawals.
          </p>
        </div>
        <Link
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <section className="rounded-sm border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-xl">Request withdrawal</h2>
          </div>
          <div className="text-sm text-muted-foreground">
            Available balance:{" "}
            <span className="font-mono font-semibold text-foreground">{fmt(balance)}</span>
          </div>
          <div className="grid gap-3">
            ...
            >
              ...
              {accounts.map((a) => (
                <option key={a.id} value={a.id}>
                  {PROVIDER_LABELS[a.provider]} — {a.display_name}
                </option>
              ))}
            </select>
            <input
            ...
            <button
              ...
              className="h-10 px-4 rounded-sm bg-primary text-primary-foreground font-semibold disabled:opacity-60 inline-flex items-center justify-center gap-2"
            >
              {busy && <Loader2 className="h-4 w-4 animate-spin" />} Submit request
            </button>
            <p className="text-xs text-muted-foreground">
              A 1.5% gateway fee and 5% withholding tax may apply on payout. All withdrawals are
              reviewed for fraud.
            </p>
          </div>
        </section>
        ...
        <section className="rounded-sm border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-xl">Add payout account</h2>
          </div>
          <div className="grid gap-3">
            <select
              value={provider}
              onChange={(e) => setProvider(e.target.value as (typeof PROVIDER_LIST)[number])}
              className="h-10 px-3 rounded-sm border border-border bg-background"
            >
              {PROVIDER_LIST.map((p) => (
            ...
            <input
              placeholder="Label (e.g. GTBank — main)"
            ...
            <textarea
              ...
              className="px-3 py-2 rounded-sm border border-border bg-background min-h-[60px]"
            />
            <button
              onClick={addAccount}
              disabled={busy}
              className="h-10 px-4 rounded-sm bg-foreground text-background font-semibold disabled:opacity-60"
            >
              Add account
            </button>
          </div>
          ...
          {accounts.map((a) => (
            <div key={a.id} className="p-4 flex items-center gap-3">
              ...
              <button
                ...
                className="text-muted-foreground hover:text-breaking"
              >
      ...
      <section className="rounded-sm border border-border bg-card">
        ...
        <div className="divide-y divide-border">
          {withdrawals.length === 0 && (
            <div className="p-6 text-sm text-muted-foreground text-center">No withdrawals yet.</div>
          )}
          {withdrawals.map((w) => (
            <div key={w.id} className="p-4 flex items-center gap-3 flex-wrap">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  Requested {new Date(w.requested_at).toLocaleString()} · fee {fmt(w.fee_kobo)} ·
                  tax {fmt(w.tax_kobo)}
                </div>
                {w.reviewer_note && (
                  <div className="text-xs text-muted-foreground mt-1">Note: {w.reviewer_note}</div>
                )}
              </div>
              {w.fraud_score >= 40 && (
                <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-sm bg-breaking/15 text-breaking">
                  Fraud {w.fraud_score}
                </span>
              )}
              <span
                className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-sm font-semibold ${STATUS_COLOR[w.status]}`}
              >
                {w.status}
              </span>
            </div>
