...
> = {
  ...
  published: {
    label: "Published",
    cls: "bg-verified text-verified-foreground",
    icon: CheckCircle2,
  },
};
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Drafts, scheduled posts and the public archive.
          </p>
        </div>
        <Link
          to="/admin/new"
      ...
      <div className="mt-6 rounded-sm border border-border overflow-hidden">
        {items === null ? (
          ...
          <div className="p-10 text-center text-muted-foreground">
            No articles yet.{" "}
            <Link to="/admin/new" className="text-primary font-semibold">
              ...
              {items.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-t border-border">
                    ...
                    <td className="p-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[11px] font-bold uppercase tracking-wide ${S.cls}`}
                      >
                        <S.icon className="h-3 w-3" /> {S.label}
                      </span>
                      {a.status === "scheduled" && a.scheduled_at && (
                        <div className="text-[11px] text-muted-foreground mt-1">
                          for {new Date(a.scheduled_at).toLocaleString()}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="p-3 text-right">
                      ...
                      <button
                        onClick={() => remove(a.id)}
                        className="ml-3 inline-flex items-center gap-1 text-destructive hover:underline"
                      >
