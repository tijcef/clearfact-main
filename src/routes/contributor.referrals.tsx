...
function ReferralsView() {
  ...
  const link =
    typeof window !== "undefined" ? `${window.location.origin}/auth?ref=${code}` : `?ref=${code}`;
  ...
  const claim = async () => {
    ...
    try {
      const res = await claimFn({
        data: { referralCode: claimCode.trim().toUpperCase(), kind: "contributor" },
      });
      toast.success(
        `Referral applied. Your inviter earned ₦${(res.reward_kobo / 100).toLocaleString()}.`,
      );
      setClaimCode("");
    } catch (e: unknown) {
    ...
    <div className="container-news py-8 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Earn ₦500 for every contributor and ₦100 for every reader you bring in.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="rounded-sm border border-border bg-card p-5">
          ...
          <div className="font-serif text-3xl mt-1 inline-flex items-center gap-2">
        ...
        <div className="rounded-sm border border-border bg-card p-5">
          ...
          <div className="font-serif text-3xl mt-1 inline-flex items-center gap-2">
            <Gift className="h-6 w-6 text-gold" />₦{(totalEarned / 100).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-sm border border-border bg-card p-5 max-w-xl">
        ...
        <div className="flex gap-2 mt-3">
          <input
            ...
            className="h-10 px-3 rounded-sm border border-border bg-background flex-1 font-mono uppercase"
          />
          <button
            onClick={claim}
            disabled={busy}
        ...
        <div className="divide-y divide-border">
          ...
          {referrals.map((r) => (
            <div key={r.id} className="p-4 flex items-center gap-3">
              ...
              <div
                className={`font-mono font-semibold ${r.rewarded ? "text-verified" : "text-muted-foreground"}`}
              >
                +₦{(r.reward_kobo / 100).toLocaleString()}
              </div>
            </div>
          ))}
        </div>
