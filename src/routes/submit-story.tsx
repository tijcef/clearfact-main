  ...
  head: () => ({
    meta: [
      { title: "Submit a Story — ClearFact News" },
      {
        name: "description",
        content:
          "Send a tip or whistleblower submission to the ClearFact News investigations desk.",
      },
    ...
    <div className="container-news py-12 md:py-16 grid lg:grid-cols-2 gap-12">
      <div>
        ...
        <p className="text-muted-foreground mt-3">
          Your identity is protected. Submissions are encrypted and reviewed by a senior editor
          only.
        </p>
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the story, share documents links, and any context…"
          required
        />
        <input
          className="h-11 w-full px-3 rounded-sm border border-border bg-background"
          placeholder="Optional contact (email or Signal)"
        />
        <button className="h-11 px-5 rounded-sm bg-gold text-gold-foreground font-semibold">
          Send securely
        </button>
      </form>
    </div>
