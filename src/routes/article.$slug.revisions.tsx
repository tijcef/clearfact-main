...
function RevisionsPage() {
  ...
  if (article === undefined)
    return <div className="container-news py-16 text-muted-foreground">Loading…</div>;
  ...
  return (
    <div className="container-news py-10 max-w-3xl">
      ...
      <p className="mt-2 text-muted-foreground">
        Every public edit to <span className="font-semibold text-foreground">{article.title}</span>{" "}
        is logged here. ClearFact News never edits silently.
      </p>

      <ol className="mt-8 relative border-l-2 border-border pl-6 space-y-6">
        {revs.length === 0 && (
          <li className="text-sm text-muted-foreground">No public revisions yet.</li>
        )}
        {revs.map((r, i) => {
          const prev = revs[i + 1];
          ...
          return (
            <li key={r.id} className="relative">
              ...
              <div className="text-xs text-muted-foreground">
                {new Date(r.created_at).toLocaleString()} · {r.editor_name ?? "Editor"}
              </div>
              ...
              {prev && (
                <ul className="mt-2 text-xs text-muted-foreground space-y-0.5">
                  {titleChanged && (
                    <li>
                      · Headline updated (was: <span className="italic">{prev.title}</span>)
                    </li>
                  )}
