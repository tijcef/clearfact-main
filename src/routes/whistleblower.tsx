...
export const Route = createFileRoute("/whistleblower")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Send confidential tips and evidence to the ClearFact investigations desk.",
      },
...
function Whistle() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (subject.length < 5 || details.length < 30)
      return toast.error("Please add a subject and detailed description (30+ chars)");
    setBusy(true);
    const { error } = await supabase
      .from("community_reports")
      .insert({ kind, subject, details, contact: contact || null, severity });
    setBusy(false);
    ...
    return (
      <div className="container-news py-20 max-w-xl text-center">
        ...
        <p className="text-muted-foreground mt-2">
          Your submission is encrypted and queued for the senior investigations editor. We do not
          log IP addresses for anonymous reports.
        </p>
      </div>
    );
  }
    ...
    <div className="container-news py-12 grid lg:grid-cols-[1fr_360px] gap-10 max-w-5xl">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-3">
          Report corruption, abuse of power, public-safety threats, environmental harm, election
          fraud or any wrongdoing. Submission is anonymous unless you choose to share contact.
        </p>
        ...
        <form onSubmit={submit} className="mt-8 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <select
              ...
              className="h-11 px-3 rounded-sm border border-border bg-background"
            >
          ...
          <textarea
            ...
            rows={10}
            placeholder="Describe what happened, when, where, who is involved. Link to documents or evidence if possible."
            required
          ...
          <input
            ...
            placeholder="Optional — Signal/email to follow up (leave blank to stay anonymous)"
            className="h-11 w-full px-3 rounded-sm border border-border bg-background"
          />
          <button
            disabled={busy}
      ...
      <aside className="space-y-4">
        <div className="rounded-sm border border-border bg-card p-5">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your tip is transmitted over TLS and stored with row-level access restricted to senior
            editors.
          </p>
        </div>
        <div className="rounded-sm border border-border bg-card p-5">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            For maximum anonymity use Tor Browser and a public Wi-Fi network. Do not include
            identifying details unless necessary.
          </p>
        </div>
      </aside>
    </div>
