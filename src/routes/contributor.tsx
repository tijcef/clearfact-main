...
export const Route = createFileRoute("/contributor")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Submit stories, manage drafts, track earnings and trust score.",
      },
    ...
    return (
      <div className="container-news py-12 max-w-xl">
        ...
        <p className="text-muted-foreground mt-2">
          Apply once. Submit stories, get verified by editors, earn payouts, build a trust score.
        </p>
        <Link
          to="/contributor/onboarding"
          className="inline-flex mt-6 h-11 items-center px-5 rounded-sm bg-primary text-primary-foreground font-semibold"
        >
          Start application
        </Link>
      </div>
        ...
        <div className="container-news h-14 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            ...
            <nav className="flex items-center gap-1 text-sm flex-wrap">
              <NavLink to="/contributor" exact icon={<LayoutDashboard className="h-4 w-4" />}>
                Dashboard
              </NavLink>
              <NavLink to="/contributor/submit" icon={<PenSquare className="h-4 w-4" />}>
                Submit
              </NavLink>
              <NavLink to="/contributor/submissions" icon={<FileText className="h-4 w-4" />}>
                My stories
              </NavLink>
              <NavLink to="/contributor/wallet" icon={<Wallet className="h-4 w-4" />}>
                Wallet
              </NavLink>
              <NavLink to="/contributor/payouts" icon={<Banknote className="h-4 w-4" />}>
                Payouts
              </NavLink>
              <NavLink to="/contributor/referrals" icon={<Gift className="h-4 w-4" />}>
                Referrals
              </NavLink>
              <NavLink to="/contributor/notifications" icon={<Bell className="h-4 w-4" />}>
                Alerts{" "}
                {unread > 0 && (
                  <span className="ml-1 px-1.5 rounded-sm bg-breaking text-breaking-foreground text-[10px]">
                    {unread}
                  </span>
                )}
              </NavLink>
          ...
          <div className="flex items-center gap-3 text-xs">
            <span
              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm font-semibold uppercase tracking-wider ${tierColor[(profile as ContribProfile).tier]}`}
            >
...
function NavLink({
  ...
  return (
    <Link
      ...
      activeProps={{
        className:
          "px-3 py-1.5 rounded-sm bg-background/15 inline-flex items-center gap-1.5 font-semibold",
      }}
    >
      {icon}
      {children}
