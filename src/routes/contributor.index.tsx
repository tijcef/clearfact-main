...
function Dashboard() {
  ...
  const stats = {
    ...
    pending: subs.filter(
      (s) => s.status === "pending" || s.status === "ai_review" || s.status === "editor_review",
    ).length,
    ...
    <div className="container-news py-8 space-y-8">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {profile.specialty.replace(/_/g, " ")} · {profile.location ?? "Nigeria"}
          </p>
        </div>
        <Link
          to="/contributor/submit"
          className="h-10 px-4 inline-flex items-center rounded-sm bg-primary text-primary-foreground font-semibold"
        >
          + New submission
        </Link>
      </div>
        ...
        <div className="border border-gold/40 bg-gold/10 rounded-sm p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Verify phone, ID and selfie to unlock paid submissions and the Verified tier.
            </p>
          </div>
          <Link
            to="/contributor/onboarding"
            className="h-9 px-3 inline-flex items-center rounded-sm bg-gold text-gold-foreground font-semibold"
          >
            Verify
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Stat
          ...
          icon={<CheckCircle2 className="h-4 w-4 text-verified" />}
        />
        <Stat
          label="In review"
          value={stats.pending}
        ...
        <div className="rounded-sm border border-border divide-y divide-border bg-card">
          {subs.length === 0 && (
            <div className="p-6 text-sm text-muted-foreground text-center">
              No submissions yet — pitch your first story.
            </div>
          )}
          {subs.map((s) => (
      ...
      <div className="rounded-sm border border-border p-5 bg-accent/40">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <TrendingUp className="h-4 w-4" /> Tier progression
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          Beginner → Trusted (3 published) → Verified (10 + trust 75) → Elite Reporter (25 + trust
          85).
        </p>
      </div>
    </div>
...
export function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
    ...
    ai_review: {
      label: "AI scan",
      cls: "bg-gold/20 text-foreground",
      icon: <Clock className="h-3 w-3" />,
    },
    editor_review: {
      label: "Editor review",
    ...
    approved: {
      label: "Approved",
      cls: "bg-verified/15 text-verified",
    ...
    rejected: {
      label: "Rejected",
      cls: "bg-breaking/15 text-breaking",
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[11px] font-semibold uppercase tracking-wider ${v.cls}`}
    >
      {v.icon}
      {v.label}
    </span>
