...
function Logo() {
  return (
    <Link to="/" className="flex items-center gap-2 group" aria-label="ClearFact News home">
      <span className="grid place-items-center h-9 w-9 rounded-sm bg-primary text-primary-foreground font-serif text-lg font-bold">
        CF
      </span>
    ...
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur border-b border-border">
      ...
      <div className="bg-primary text-primary-foreground text-xs">
        <div className="container-news flex h-8 items-center justify-between">
          <span className="hidden sm:inline">{date} · Yola, Nigeria</span>
          <div className="flex items-center gap-4">
            ...
            <Link to="/contribute" className="hover:text-gold">Become a Contributor</Link>
            <Link to="/whistleblower" className="hover:text-gold">Tip Line</Link>
            <Link to="/dashboard" className="hover:text-gold">My Account</Link>
            <ThemeToggle />
          </div>
        </div>
      </div>
      ...
      {/* Category nav */}
      <nav className={`border-t border-border ${open ? "block" : "hidden lg:block"}`} aria-label="Sections">
        <div className="container-news flex flex-wrap lg:flex-nowrap gap-x-1 gap-y-1 lg:gap-x-3 overflow-x-auto py-2 text-sm">
          {CATEGORIES.map((c) => (
            <Link
              ...
              activeProps={{ className: "px-2 py-1 rounded-sm whitespace-nowrap font-semibold text-primary bg-accent" }}
            >
              {c.label}
            </Link>
          ))}
      ...
      <div className="bg-foreground text-background overflow-hidden">
        <div className="container-news flex items-center gap-3 py-1.5">
          <span className="shrink-0 inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider bg-breaking text-breaking-foreground px-2 py-0.5 rounded-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-breaking-foreground animate-pulse" /> Live
          </span>
