...
const screenSchema = z.object({
  ...
  plagiarism_score: z
    ...
    .min(0)
    .max(100)
    .describe("Estimated probability the text is recycled or paraphrased from public sources"),
  duplicate_risk: z
    .enum(["none", "low", "medium", "high"])
    .describe("Risk that the same story has been recently covered"),
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.decision === "reject") {
      ...
      const { data: prof } = await supabase
        ...
        .select("rejected_count,trust_score")
      ...
      await supabase.from("notifications").insert({
        ...
        body:
          data.feedback ??
          "An editor has reviewed your submission and could not approve it at this time.",
    ...
    if (payout > 0) {
      ...
      const { data: prof } = await supabase
        .from("contributor_profiles")
        .select("wallet_balance_kobo,accepted_count,trust_score,tier")
        .eq("user_id", sub.contributor_id)
        .maybeSingle();
      if (prof) {
        ...
        const newTier =
          newAccepted >= 25 && newTrust >= 85
            ? "elite"
            : newAccepted >= 10 && newTrust >= 75
              ? "verified"
              : newAccepted >= 3
                ? "trusted"
                : "beginner";
